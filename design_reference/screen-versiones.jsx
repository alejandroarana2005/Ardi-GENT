/* global React, Panel, Btn, Tag */
const { useState: useStateV } = React;

window.ScreenVersiones = function ScreenVersiones({ data }) {
  const [a, setA] = useStateV("sch_2024A_v07");
  const [b, setB] = useStateV("sch_2024A_v06");
    ...
    <div className="page">
      ...
      <Panel title="Línea de tiempo del semestre" meta={data.VERSIONES.length + " versiones"} style={{ marginBottom: 18 }}>
        <div style={{ position: "relative", padding: "24px 0 4px" }}>
          <div style={{
            position: "absolute", top: 36, left: 12, right: 12, height: 1, background: "var(--border)"
          }}></div>
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${data.VERSIONES.length}, 1fr)`, gap: 0 }}>
            {[...data.VERSIONES].reverse().map((v, i) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginBottom: 18 }}>
        <VersionCard v={verA} role="A" data={data} />
        <VersionCard v={verB} role="B" data={data} />
      </div>
...
function VersionCard({ v, role, data }) {
  return (
    <div className="panel">
