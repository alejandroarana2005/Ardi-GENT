/* global React, Panel, Tag, Pill, Btn, StatusDot */
...
window.ScreenConflictos = function ScreenConflictos({ data }) {
  const [sel, setSel] = useStateC(data.CONFLICTOS[0]);
  const [stage, setStage] = useStateC(0); // 0 detected, 1 diagnose, 2 propose, 3 applied
  ...
  return (
    <div className="page">
      ...
      <div className="kpi-grid" style={{ marginBottom: 18 }}>
        <KPIc label="Conflictos abiertos"     value="4"     unit="" delta="2 nuevos hoy"           kind="conflict" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: 18, alignItems: "start" }}>
        <Panel title="Cola de eventos" meta={data.CONFLICTOS.length + " abiertos"} flush>
          {data.CONFLICTOS.map(c => {
            ...
            return (
              <button key={c.id}
                onClick={() => { setSel(c); setStage(0); }}
                style={{
                  width: "100%", textAlign: "left",
                  padding: "14px 18px", border: "none", background: active ? "var(--surface-2)" : "var(--surface)",
                ...
                <div style={{ marginTop: 6, fontSize: 14 }}>{TYPE_LABELS[c.type]}</div>
          ...
          <Panel title={"Reparación · " + sel.id} meta={"event_type · " + sel.type}>
            ...
            <div style={{ marginTop: 22, display: "flex", justifyContent: "flex-end", gap: 8 }}>
              ...
              <Btn kind="primary" icon="▸" onClick={() => setStage(s => Math.min(3, s + 1))}>
                {stage === 0 ? "Diagnosticar" : stage === 1 ? "Generar propuesta" : stage === 2 ? "Aplicar reparación" : "Cerrar evento"}
              </Btn>
            </div>
          </Panel>
...
function KPIc({ label, value, unit, delta, kind }) {
  const color = kind === "conflict" ? "var(--conflict)" : kind === "ok" ? "var(--ok)" : kind === "warn" ? "var(--warn)" : "var(--ink)";
  return (
...
function Plan({ rows }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {rows.map((r, i) => (
        <div key={i} style={{
          padding: 8, border: "1px solid " + (r.changed ? "var(--agent)" : "var(--border)"),
