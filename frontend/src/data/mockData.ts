export interface Prof {
  code: string; name: string; contract: string; load: number; max: number;
}
export interface Aula {
  code: string; name: string; capacity: number; resources: string[];
}
export interface Materia {
  code: string; name: string; prof: string; grupos: number;
  sesiones: number; mat: number; fac: string;
}
export interface Slot { code: string; label: string; }
export interface Asignacion {
  mat: string; g: number; s: number; aula: string;
  day: string; slot: string; prof: string; u: number;
}
export interface Restriccion {
  code: string; name: string; type: "hard" | "soft";
  desc: string; active: boolean; satisf: number;
}
export interface Conflicto {
  id: string; type: string; subject: string;
  afected: number; ts: string; severity: "alta" | "media" | "baja"; desc: string;
}
export interface PipelineLayer {
  n: number; key: string; name: string; desc: string;
  status: "ok" | "run" | "wait" | "fail"; ms: number;
  detail: Record<string, string | number | boolean>;
}
export interface Desire {
  code: string; name: string; weight: number; status: string;
}
export interface Intention {
  code: string; step: string; status: string; t: string;
}
export interface AHP {
  weights: { w1: number; w2: number; w3: number; w4: number };
  lambda_: number; cr: number;
  matrix: number[][]; labels: string[];
}
export interface Version {
  id: string; parent: string | null; solver: string; U: number;
  t: string; date: string; note: string; active: boolean;
  hcv: number; scv: number;
}
export interface HaiaData {
  SEMESTRE: string; PROFS: Prof[]; AULAS: Aula[]; MATERIAS: Materia[];
  DAYS: string[]; SLOTS: Slot[]; ASIGNACIONES: Asignacion[];
  RESTRICCIONES: Restriccion[]; CONFLICTOS: Conflicto[];
  PIPELINE: PipelineLayer[]; BELIEFS: string[];
  DESIRES: Desire[]; INTENTIONS: Intention[];
  AHP: AHP; VERSIONES: Version[];
}
...
export const mockData: HaiaData = {
